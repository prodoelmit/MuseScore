                            ...
                            bool forceAccidental, const std::set<SymId>& articulationIds, bool rhythmic, InputState* externalInputState)
{
    ...
    for (;;) {
        ...
        Fraction totalTupletRatio = cr->ticks() / cr->globalTicks();
...
//
//    _desiredDuration should be in ticks (local to tuple), not globalTicks
//    e.g. When removing 1/4 from triplet - you need to pass 1/4, not 2/12
//
...
Fraction Score::makeGap(Segment* segment, track_idx_t track, const Fraction& _desiredDuration, Tuplet* tuplet, bool keepChord)
{
    ...
    for (Segment* currSegment = firstSegment; currSegment; currSegment = currSegment->next(SegmentType::ChordRest)) {
        ...
        if (desiredDurationCopy < removedTicks) {
            ...
            LOGI("Need to remove %d/%d,\t gonna remove %d/%d,\t last tick %d/%d", desiredDurationCopy.numerator(), desiredDurationCopy.denominator(),
            ...
            if (tuplet) {
                Fraction totalRatioOfCurrCR = currCR->ticks() / currCR->globalTicks();
                // We need to normalize it to tuplet's internal durations
                dList = toDurationList(overdeletedTicks * totalRatioOfCurrCR, false);
            ...
            for (TDuration duration : dList) {
                restoredTicks += duration.ticks();
                LOGI("%d/%d", duration.ticks().numerator(), duration.ticks().denominator());
                if (ltuplet) {
...
bool Score::makeGapVoice(Segment* seg, track_idx_t track, Fraction len, const Fraction& tick, Tuplet* tuplet)
{
    ...
    if (!cr) {
        ...
        Fraction totalTupletRatio = cr1->ticks() / cr1->globalTicks();
...
Segment* Score::setChord(Segment* segment, track_idx_t track, const Chord* chordTemplate, Fraction dur)
{
    ...
    for (;;) {
        ...
        totalTupletRatio = cr->ticks() / cr->globalTicks();
